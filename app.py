...
@app.route('/api/profile/monthly-stats', methods=['GET'])
def api_monthly_stats():
    """Get aggregated statistics for the current month"""
    user_id = get_default_user()
    conn = sqlite3.connect('eco_twin.db')
    c = conn.cursor()
    ...
    return jsonify({
        'electricity': row[0] if row else 0,
        'transport': row[1] if row else 0,
        'industries': row[2] if row else 0,
        'waste': row[3] if row else 0,
        'total_co2': row[4] if row else 0
    })
